function [D, order] = ambiDecoder_mod(ls_dirs, method, rE_WEIGHT, order)
...
%   localization blur.
%
%   12/01/2023; Modified EPAD to accept undersampled decoding directions. 
%               M. McCrea
%
% Inputs:   
...
switch lower(method)
    case 'sad'
        D = (4*pi)/ls_num * Y_ls.'; % TODO: this should prob be / nSH. See shd_gain_modification_test.m
    case 'mmd'
        D = pinv(Y_ls);
    case 'epad'
        [U,~,V] = svd(Y_ls);
        V_trunc = V(:,1:min((order+1)^2,ls_num));
        D = (4*pi)/ls_num * V_trunc*U(:,1:min((order+1)^2,ls_num)).';
    case 'allrad'
